...
5. [ ] **データセーフティフォーム**: 本アプリの実装での回答目安
   - 位置情報: 端末内にのみ保存し開発者へ送信しない → 「収集」に**該当しない**
   - ただし地図タイル取得で IP アドレスとタイル座標が OSM サーバーへ送られる。
     保守的に申告するなら「おおよその位置 / アプリの機能のため / 第三者へ送信」を選択
   - 広告・解析・トラッキング: なし
...
※ 位置情報の「機密性の高い権限申告フォーム」は**バックグラウンド位置情報を使う場合のみ**必要。
   本アプリはフォアグラウンドのみなので不要。
